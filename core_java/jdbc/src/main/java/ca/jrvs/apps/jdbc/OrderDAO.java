...
public class OrderDAO extends DataAccessObject<Order> {
  ...
  private final static String GET_BY_ID = "SELECT c.first_name, c.last_name, c.email, o.order_id, "
      ...
      + " p.code, p.name, p.size, p.variety, p.price FROM orders o JOIN customer c "
      + "ON o.customer_id=c.customer_id JOIN salesperson s ON o.salesperson_id=s.salesperson_id "
  ...
  private final static Logger logger = LoggerFactory.getLogger(JDBCExecuter.class);
  ...
  public Order findById(long id) {
    ...
    try(PreparedStatement statement = this.connection.prepareStatement(GET_BY_ID)) {
      ...
      logger.error("ERROR: Could not find order with given ID.");
  ...
  public List<Order> getOrdersForCustomer(long customerId) {
    ...
    try(PreparedStatement statement = this.connection.prepareStatement(GET_FOR_CUST)) {
      ...
      logger.error("ERROR: Could not get orders for given customer ID.", ex);
    }
    return orders;
  }
}
