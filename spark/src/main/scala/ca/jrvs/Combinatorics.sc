...
val flatMapper = numList.flatMap(i => i.map(i => i*2))
val mapFlattend = numList.map(i => i.map(i => i*2)).flatten
...
val unsumedList = List.range(1, 11)
val sumList = unsumedList.sum
val reduceSum = unsumedList.reduce(_ + _)
val foldLeftSum = unsumedList.foldLeft(0.0)(_ + _)
...
countryMap.getOrElse("edward", "not here")
countryMap.getOrElse("Sam", "n/a")
...
val numberedName = names2.map(name => (name, names2.indexOf(name) + 1))
